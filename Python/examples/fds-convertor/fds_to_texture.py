...
def process_single_case(case_info):
    ...
    output_path = output_path / case_name
    ...
    for file_id, q_file in enumerate(q_files):
        ...
        plot3d_img.save(img_buffer, format='PNG')
        
        # Get history data
        history_devc = get_history_data(args, devc_data, file_id)

        # Extract time info
        _, seconds, milliseconds  = extract_time(q_file.stem)
    ...
    # More information please check https://huggingface.co/docs/dataset-viewer/en/parquet
...
if __name__ == '__main__':
    ...
    # Create README.md
    with open(args.output_dataset_dir / "README.md", 'w') as f:
        f.write(f"""---
data_files:
- split: train
  path: "train/*/*.parquet"
...
This dataset was generated by converting FDS simulation data into volume texture format.
      """)
