...
if __name__ == '__main__':
    ...
    parser.add_argument('--texture_data_dir', default=str((Path(__file__).parent / '../../data/cube-texture-v2').resolve()), type=str)
            ...
            captions.append(
                {
                    ...
                    "text": f"[T]={train_devc['Time']:.2f}; " + \
                            '[HD]=' + ', '.join([f"{float(devc):.2f}" for col, devc in zip(train_devc.index, train_devc) if col != 'Time'])
                }
            )
