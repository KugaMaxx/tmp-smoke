...
class VQTokenizer(PreTrainedTokenizer):
    ...
    def _tokenize(self, text: Union[str, list[list[float]]]) -> torch.Tensor:
        ...
        inputs = self.convert_string_to_tensor(text, is_norm=True, mean=78, std=78) # TODO check
    ...
    def convert_string_to_tensor(self, text: Union[str, list[list[float]]], is_norm: bool = True, 
                                 mean: Optional[Union[float, list, torch.Tensor]] = 78,
                                 std: Optional[Union[float, list, torch.Tensor]] = 78) -> torch.Tensor:
        """
        Convert a string or list of lists to a tensor for tokenization.
        
        Args:
            text: Input data as string or list of lists
            is_norm: Whether to apply normalization
            mean: Mean for normalization. If scalar, applies globally; if multi-dimensional, applies per dimension
            std: Standard deviation for normalization. If scalar, applies globally; if multi-dimensional, applies per dimension
        """
        ...
        if is_norm:
            if mean is not None and std is not None:
                ...
                mean_tensor = torch.tensor(mean, dtype=torch.float32) if not isinstance(mean, torch.Tensor) else mean
                std_tensor = torch.tensor(std, dtype=torch.float32) if not isinstance(std, torch.Tensor) else std
                
                # Ensure tensors have correct shape for broadcasting
                if mean_tensor.dim() == 0:  # scalar
                    mean_tensor = mean_tensor.expand_as(inputs)
                elif mean_tensor.dim() == 1:  # per-dimension
                    mean_tensor = mean_tensor.unsqueeze(0).expand_as(inputs)
                
                if std_tensor.dim() == 0:  # scalar
                    std_tensor = std_tensor.expand_as(inputs)
                elif std_tensor.dim() == 1:  # per-dimension
                    std_tensor = std_tensor.unsqueeze(0).expand_as(inputs)
                
                inputs = (inputs - mean_tensor) / (std_tensor + 1e-8)  # Add small epsilon to avoid division by zero
            else:
                # Use RevIN for input normalization
                revin = RevIN(len(text[0]), eps=1e-5, affine=False)
                inputs = revin(inputs, mode='norm')

        return inputs

    def run_pipeline(self, text: Union[str, list[list[float]]], is_norm: bool = True,
                     mean: Optional[Union[float, list, torch.Tensor]] = 78,
                     std: Optional[Union[float, list, torch.Tensor]] = 78) -> torch.Tensor:
        """
        Convert a string or list of lists to a tensor for tokenization and run through the VQ model.
        ...
        """
        # Convert input string or list to tensor
        inputs = self.convert_string_to_tensor(text, is_norm=False, mean=None, std=None)
        ...
        if use_custom_norm and mean is not None and std is not None:
